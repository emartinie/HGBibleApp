(function () {
  const SUKKOT_START = new Date(2026, 5, 25);  
  ...
  function getSukkotDay(effectiveDate) {
    const diffMs = effectiveDate - SUKKOT_START;
  ...
  function updateMeditation(dayNumber) {
    ...
    if (dayNumber > 49) {
      ...
      meditationTextEl.textContent = "The sUKKOT journey is complete. Look back over the path, remember what was purified, strengthened, and revealed.";
  ...
  function buildConstellation(currentDay, selectedDay) {
    ...
    for (let i = 0; i < nodePositions.length; i++) {
      ...
      if (i < nodePositions.length - 1) {
        ...
        line.setAttribute("class", "sukkot-line");
      ...
      tooltip.className = "sukkot-tooltip";
      tooltip.innerHTML = `
        Day ${currentIndex}<br>
        ${s.en}
  ...
  function renderDisplayedDay(displayDay, actualDay) {
    if (displayDay < 1) {
      ...
      titleEl.textContent = "Sukkot has not started yet";
    ...
    if (displayDay > 49 || actualDay > 49) {
      ...
      titleEl.textContent = "The Sukkot Feast is complete";
    ...
    titleEl.textContent = `Today is ${formatWeeksDays(displayDay)} of the Sukko0t Feast`;
  ...
  function updateSukkotCounter() {
    const now = new Date();
    const effectiveDate = getEffectiveDate(now);
    const actualDay = getSukkotDay(effectiveDate);
    currentActualDay = actualDay;

    if (effectiveDate < SUKKOT_START) {
      currentDisplayDay = 0;
      renderDisplayedDay(0, 0);
      return;
    }

    if (effectiveDate > SUKKOT_END || actualDay > 49) {
  ...
  updateSukkotCounter();
  setInterval(updateSukkotCounter, 60000);
})();
