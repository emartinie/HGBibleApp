...
:: Week 1 template file
set source="Week 1- In the Beginning B’reisheet (בראשית).docx"

:: Weekly titles: English Name Translation + Hebrew Transliteration|Hebrew
...
:: Loop through weeks 2–52
for /L %%i in (2,1,52) do (
    for /f "tokens=1,2 delims=|" %%a in ("!titles[%%i]!") do (
        set en=%%a
        set he=%%b
        copy %source% "Week %%i- !en! (!he!).docx" >nul
        echo Created: Week %%i- !en! (!he!).docx
    )
)

echo ✅ All weekly files created successfully!
pause
